...
[Quick start](docs/getting-started.md) | [Documentation](docs/README.md) | [Changelog](CHANGELOG.md) | [Code examples](https://github.com/redisson/redisson-examples) | [Report an issue](https://github.com/redisson/redisson/issues/new)
...
Complete in-repo path: **install** (Maven/Gradle) then **call/run** (`Redisson.create` + objects). Follow [docs/getting-started.md](docs/getting-started.md). Also listed from [docs/README.md](docs/README.md).

### 1. Get / install

Add the Community Edition artifact (see [docs/getting-started.md](docs/getting-started.md) for PRO and Gradle):
...
From this repository (JDK 8+, Maven 3.0.5+):

```bash
git clone https://github.com/redisson/redisson.git
cd redisson
mvn -pl redisson -am package -DskipTests
```

### 2. Call / run

Requires a local Valkey/Redis at `127.0.0.1:6379` (or change the address). Full listing is in [docs/getting-started.md](docs/getting-started.md).
...
Run the module tests from this repo (profile `unit-test` enables Surefire):

```bash
mvn -pl redisson -am test -Punit-test
```
