...
In-repository usage path (get/install + call/run): **[getting-started.md](getting-started.md)**

1. Add the Maven or Gradle dependency (`org.redisson:redisson`).
2. Create `Config`, call `Redisson.create(config)`, then use objects such as `getMap` / `getLock`.

Root README points here and to [getting-started.md](getting-started.md).
